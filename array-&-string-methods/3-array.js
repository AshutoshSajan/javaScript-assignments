...
function doesWordExist(arry, word){
  var elm = arry.filter(value => value == word)
   if(elm.toString().includes(word)){
    return true;
  } else {
    return false;
  };
...
function howManyTimes(arry = [], word){
  var elm = arry.filter(value => value == word)
  return `${elm.length} times`;
}
...
var pop = array.filter(value => value.country !== "China");
  return pop.reduce((acc, val) => {
    return acc += val.pop;
  }, 0);
}
...
a.reduce((acc, val) => {
  acc.push(val*2) // bacause push is a method which returns the array length so if we use retern here then it will give us error;
  return acc;//returns the acc value
}, [])